...
function Cache(filepath) {
    ...
    function cache_value(query_key, value) {
        if (!cache) {
            return value;
        }

        var watcher_key = data.filepath + '.iswatched',
            keys_key = data.filepath + '.keys';

        if (cache[watcher_key] === undefined) {
            watch(data.filepath, function (file) {
    ...
    function cache_lookup(query_key) {
...
function query(ledgerfile, args, optional_runner, cache) {
    ...
    function cached_run(ledger, params) {
        var value = cache.find(query_key);
    ...
    return cached_run('ledger', params).then(function (streams) {
        return cache.find(query_key) ||
            cache.put(query_key, xml_stream(streams[0]));
    });
}
